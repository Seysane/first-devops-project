...
## 4. Automated Service Guard (Cron Watcher Script)

The objective of this exercise is to understand how automated service monitoring and self-healing loops work in a Linux environment. We deploy a local bash script that checks the status of the `cron` daemon, restarts it if it is stopped, and logs the outcome.

### Step 1 – Deploying the Monitoring Script

Using the `tee` and Here-Doc (`<< 'EOF'`) method, I injected the automation script into the system's local binary directory and granted it executable permissions:

```bash
sudo tee /usr/local/bin/cron-watcher.sh > /dev/null << 'EOF'
#!/bin/bash
if ! systemctl is-active --quiet cron; then
  echo "$(date): cron does not work, restarting..." >> /var/log/cron-watcher.log
  systemctl start cron
else
  echo "$(date): cron works correctly" >> /var/log/cron-watcher.log
fi
EOF
```
...
if ! systemctl is-active --quiet cron; then, this is the core conditional check.

systemctl is-active --quiet cron checks if the cron service is currently running. The --quiet flag suppresses standard output, making the command silent; it only returns an exit code (0 for active, non-zero for inactive).

The exclamation mark (!) represents logical negation (NOT). Therefore, the condition translates to: "If the cron service is NOT active, execute the following block."

systemctl start cron - The self-healing mechanism. If the service was found dead, the script automatically attempts to spin it back up.

>> /var/log/cron-watcher.log - Appends a timestamped log entry to a custom log file, ensuring historical tracking of the script's interventions.

### Step 2 – Testing the Self-Healing Mechanism

To test whether the automated guard performs correctly under failure conditions, I manually stopped the cron service, triggered the script, and inspected the results.


### 1. Simulate a service failure by stopping cron

```bash
sane@power-sane:~$ sudo systemctl stop cron
```
### 2. Execute the watcher script manually to trigger the recovery loop

```bash
sane@power-sane:~$ sudo /usr/local/bin/cron-watcher.sh
```

### 3. Verify the logs generated by the script

```bash
sane@power-sane:~$ cat /var/log/cron-watcher.log
Fri May 29 05:18:12 PM CEST 2026: cron does not work, restarting..
```

### 4. Check systemd status to confirm cron was successfully resurrected

```bash
sane@power-sane:~$ sudo systemctl status cron
● cron.service - Regular background program processing daemon
     Loaded: loaded (/lib/systemd/system/cron.service; enabled; preset: enabled)
     Active: active (running) since Fri 2026-05-29 15:18:12 CEST; 4s ago
   Main PID: 278912 (cron)
```
